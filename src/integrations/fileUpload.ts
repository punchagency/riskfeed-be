...
class FileUploadService {
    ...
    private fileFilter = (_req: Request, file: multer, cb: FileFilterCallback): void => {
    ...
    public getMulterOptions() {
        return {
            storage: multerS3({
                ...
                key: (_req: Request, file: multer, cb) => {
                    const fileName = this.generateFileName(file.originalname);
                    const key = `${file.fieldname}/${fileName}`;
                    cb(null, key);
                },
                metadata: (req: Request, file: multer, cb) => {
                    cb(null, {
    ...
    public async generatePresignedUrl(key: string, expiresIn: number = 3600): Promise<string | null> {
        const [error, url] = await catchError<string>(
